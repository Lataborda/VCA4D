...
def main():
    ...
    with colf1:
        selected_sources   = st.multiselect("Select files",   available_sources,   default=available_sources)
    with colf2:
        selected_entities  = st.multiselect(f"Select {comparison_label.lower()}s",
                                            available_entities, default=available_entities)
    with colf3:
        ...
        selected_categories = st.multiselect("Select impact categories",
                                             available_categories, default=default_categories)
    ...
    chart_type = st.radio("Chart type", ["Horizontal bar chart", "Radar chart", "Heatmap"], horizontal=True)
    ...
    if reference_file is not None:
        try:
            ...
            if pb_merged.empty:
                ...
                with c1: x_max = st.number_input("X-axis maximum", 1.5, 20.0, float(default_xmax), 0.5)
                with c2: warning_limit = st.number_input("Second visual threshold", 1.1, 20.0, 2.0, 0.1)
                with c3:
                    dt = (f"{pb_entity} vs system limits" if chart_mode == "Single product"
                          else "Comparaison des produits vs limites du système")
                    chart_title = st.text_input("Chart title", value=dt)
