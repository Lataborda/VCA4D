            ...
            def create_stacked_bar_chart(df, selected_categories, selected_locations):
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(
                        ...
                        ticktext=categories,
                        tickfont=dict(size=18),# Aumentar el tamaño de las etiquetas
                    ),
                    showlegend=True,
...
with PR:
    if DD == "**Production de racines de manioc**":
            ...
            # Selector múltiple para las columnas (localidades)
            ...
            # Selector múltiple para las categorías de impacto
            options_categories = st.multiselect(
                "Sélectionnez les catégories d'impact à afficher :",
                list(df_normalized.index),
                default=[
                    "Climate change (kg CO2 eq)", "Freshwater eutrophication (kg P eq)", 
                    "Terrestrial ecotoxicity (kg 1,4-DB eq)", "Natural land transformation (m2)"
